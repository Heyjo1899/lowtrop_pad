        ...
        if date_folder.is_dir() and re.fullmatch(r"\d{8}", date_folder.name):
            ...
            for csv_file in date_folder.glob("*.csv"):
                try:
                    ...
                    if missing_columns:
                        print(
                            f"  Warning: Skipping {csv_file.name} in {date_folder.name} due to missing columns: {', '.join(missing_columns)}"
                        )
                        continue
                    ...
                    if "xq2_time" in df_processed.columns:
                        try:
                            ...
                            df_processed["xq2_time"] = pd.to_datetime(
                                df_processed["xq2_time"], errors="coerce"
                            )
                            # Drop rows where conversion failed (NaT) for 'xq2_time'
                            df_processed = df_processed.dropna(subset=["xq2_time"])
                            df_processed["xq2_time"] = df_processed[
                                "xq2_time"
                            ].dt.strftime("%Y-%m-%d %H:%M")
                        except Exception as time_e:
                            print(
                                f"  Warning: Could not format 'xq2_time' in {csv_file.name}: {time_e}. Keeping original format for this column."
                            )
                    ...
                    original_filename = (
                        csv_file.name
                    )  # e.g., merged_20230802-4-1-1-validation_control_tundra.csv
                    ...
                    match = re.search(
                        r"merged_(\d{8})-(\d+)-\d+-(\d+)-(.+?)\.csv", original_filename
                    )
                    ...
                    if match:
                        ...
                        full_surface_string = match.group(
                            4
                        )  # e.g., "validation_control_tundra"
                        ...
                        print(
                            f"  Warning: Could not parse filename pattern for {original_filename}. Using default naming."
                        )
