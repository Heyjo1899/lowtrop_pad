...
def plot_era5_synoptic(
    date, time, path_to_file, variable_to_plot, level, output_path, output_filename=None
):
    """
    ...
    contour_levels = np.linspace(
        data_height.min().values, data_height.max().values, num=10
    )
    contours = ax.contour(
        data_height.longitude,
        data_height.latitude,
        data_height.values,
