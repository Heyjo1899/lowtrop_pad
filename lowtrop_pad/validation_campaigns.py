...
def load_save_mast_xq2_ascents(
    ...
    times.to_csv(
        "data//mast_experiment//single_experiments//times_mast_campaigns.csv",
        index=False,
    )
...
def mast_data_df(mode):
    ...
    # Define directories
    mast_directory = os.path.join(
        "data", "mast_experiment", "single_experiments", "mast_data_single_exp"
    )
    xq2_directory = os.path.join(
        "data", "mast_experiment", "single_experiments", f"single_{mode}"
    )
    hl_directory = os.path.join(
        "data", "mast_experiment", "single_experiments", "humilog_single_exp"
    )
            ...
            if filename.endswith(".csv") and filename.startswith(f"{number}_mast"):
                ...
                if filename.startswith("5_mast"):
                    altitudes = [
                        ...
                        20,
                        85,
                    ]  # Extract altitudes 0 and 4.5 for 5_mast
                else:
                    altitudes = [2, 8, 20, 85]
                ...
                for alt_ag in altitudes:
                    ...
                    if not row.empty:
                        new_row = {
                            ...
                            "xq2_t": row["t"].values[
                                0
                            ],  # Add 't' value to the xq2_t column
                            "xq2_h": row["h"].values[
                                0
                            ],  # Add 'h' value to the xq2_h column
        ...
        for filename in os.listdir(mast_directory):
            if filename.endswith(".csv") and filename.startswith(f"{number}_mast"):
                ...
                for index, row in result_df[result_df["alt_ag"] == alt_ag].iterrows():
                    closest_time_idx = (df["time"] - row["time"]).abs().idxmin()
                    closest_time_row = df.loc[closest_time_idx]
                    result_df.at[index, "station_t"] = closest_time_row[
                        "t"
                    ]  # Add 't' value to the station_t column
                    result_df.at[index, "station_h"] = closest_time_row[
                        "h"
                    ]  # Add 'h' value to the station_h column
...
def calculate_differences(
    mast_data_df_path, mode, output_dir="data//mast_experiment//results"
):
    """
    ...
    results_df.to_html(
        os.path.join(output_dir, f"differences_{mode}.html"), index=False
    )
...
def calculate_metrics(
    ...
    # Initialize an empty DataFrame to store the results
    results_df = pd.DataFrame(
        columns=["alt_ag", "pearson_corr_t", "rmse_t", "pearson_corr_h", "rmse_h"]
    )
    ...
    for alt_ag in alt_ag_levels:
        ...
        rmse_t = np.sqrt(
            mean_squared_error(df_filtered["xq2_t"], df_filtered["station_t"])
        )
        rmse_h = np.sqrt(
            mean_squared_error(df_filtered["xq2_h"], df_filtered["station_h"])
        )
    ...
    overall_rmse_t = np.sqrt(
        mean_squared_error(final_df["xq2_t"], final_df["station_t"])
    )
    overall_rmse_h = np.sqrt(
        mean_squared_error(final_df["xq2_h"], final_df["station_h"])
    )
    ...
    results_df.to_html(
        os.path.join(output_dir, f"stat_metrics_{mode}.html"), index=False
    )
...
def plot_wind_speed_impact(
    descents_mast_data_df_path, ascents_mast_data_df_path, output_dir
):
    """
    ...
    final_df = pd.DataFrame(
        columns=["alt_ag", "time", "diff_t", "diff_h", "wind_speed_20m"]
    )
    ...
    colors = (
        plt.cm.tab10.colors
    )  # Use 10 discrete colors from matplotlib's tab10 colormap
    ...
    for idx, alt_ag in enumerate(unique_alt_ag):
        subset = final_df[final_df["alt_ag"] == alt_ag]
        plt.scatter(
            final_df_ascents.loc[subset.index, "xq2_t"],
            final_df_descents.loc[subset.index, "xq2_t"],
            color=colors[idx % len(colors)],
            label=f"{alt_ag}m",
            marker="x",
            s=10,
    ...
    plt.savefig(
        os.path.join(
            ...
            "ascents_vs_descents",
            "ascent_vs_descent_temperature.png",
        )
    )
    plt.close()
    ...
    for idx, alt_ag in enumerate(unique_alt_ag):
        subset = final_df[final_df["alt_ag"] == alt_ag]
        plt.scatter(
            final_df_ascents.loc[subset.index, "xq2_h"],
            final_df_descents.loc[subset.index, "xq2_h"],
            color=colors[idx % len(colors)],
            label=f"{alt_ag}m",
            marker="x",
            s=10,
    ...
    os.makedirs(
        os.path.join(os.path.dirname(output_dir), "ascents_vs_descents"), exist_ok=True
    )
    ...
    plt.savefig(
        os.path.join(
            os.path.dirname(output_dir),
            "ascents_vs_descents",
            "ascent_vs_descent_humidity.png",
        )
    )
    plt.close()


def uncertainty_thresholds(
    file_path_ascents, file_path_descents, thresholds_t, thresholds_h, output_dir
):
    """
    ...
    for threshold in thresholds_t:
        percent_diff_t_ascents = (
            (df_ascents["abs_diff_t"] <= threshold).sum()
            / total_points_df_ascents_t
            * 100
        )
        percent_diff_t_descents = (
            (df_descents["abs_diff_t"] <= threshold).sum()
            / total_points_df_descents_t
            * 100
        )
        results_t.append(
            {
    ...
    for threshold in thresholds_h:
        percent_diff_h_ascents = (
            (df_ascents["abs_diff_h"] <= threshold).sum()
            / total_points_df_ascents_h
            * 100
        )
        percent_diff_h_descents = (
            (df_descents["abs_diff_h"] <= threshold).sum()
            / total_points_df_descents_h
            * 100
        )
        results_h.append(
            {
    ...
    plt.plot(
        results_df_t["Threshold"],
        results_df_t["% Diff Ascents <= Threshold"],
        label="T Agreement Ascents",
        marker="o",
    )
    plt.plot(
        results_df_t["Threshold"],
        results_df_t["% Diff Descents <= Threshold"],
        label="T Agreement Descents",
        marker="s",
    )
    ...
    plt.figure(figsize=(10, 6))
    plt.plot(
        results_df_h["Threshold"],
        results_df_h["% Diff Ascents <= Threshold"],
        label="H Agreement Ascents",
        marker="o",
    )
    plt.plot(
        results_df_h["Threshold"],
        results_df_h["% Diff Descents <= Threshold"],
        label="H Agreement Descents",
        marker="s",
    )
    ...
    results_df_t.to_html(
        "data//mast_experiment//results//agreement_abs_diff_t.html", index=False
    )
    results_df_h.to_html(
        "data//mast_experiment//results//agreement_abs_diff_h.html", index=False
    )
...
def diff_ascent_descent(ascents_dir, descents_dir, output_dir):
    ...
    h_differences["mean_h_diff"] = (
        h_differences[1:].iloc[:, 1:].mean(axis=1, skipna=True)
    )
    ...
    plt.figure(figsize=(10, 6))
    plt.plot(
        t_differences["mean_t_diff"],
        t_differences["alt_ag"],
        label="Mean Temperature Difference (°C)",
    )
    plt.fill_betweenx(
        t_differences["alt_ag"],
    ...
    plt.plot(
        h_differences["mean_h_diff"],
        h_differences["alt_ag"],
        label="Mean Humidity Difference (%)",
    )
    plt.fill_betweenx(
        h_differences["alt_ag"],
...
def all_diffs_vs_wind_scatter(ascents_dir, descents_dir, wind_file_path, output_dir):
    ...
    for file_name in os.listdir(ascents_dir):
        if file_name.endswith(".csv"):
            ascent_file_path = os.path.join(ascents_dir, file_name)
            descent_file_path = os.path.join(
                descents_dir, file_name.replace("ascent", "descent")
            )
    ...
    plt.figure(figsize=(10, 6))
    scatter = plt.scatter(
        t_differences["wind_speed"],
        t_differences["t_diff"],
        c=t_differences["alt_ag"],
        cmap="Spectral",
        s=1,
    )
