...
new_directory = (
    r"C:\Users\jonat\OneDrive - Universität Graz\MASTERARBEIT\Analysis\lowtrop_pad"
)
...
xq_2_path = "data/xq2/make_Gif_XQ2_0801"
carra_path = "data/reanalysis/make_Gif_CARRA_0801"
...
def get_wind_direction_at_time(time, df_asiaq, tolerance=pd.Timedelta(minutes=5)):
    nearest_times = df_asiaq.index[
        (df_asiaq.index >= (time - tolerance)) & (df_asiaq.index <= (time + tolerance))
    ]
...
for i, (carra_file, carra_time) in enumerate(carra_files_with_times):
    ...
    if os.path.exists(xq2_filepath):
        ...
        wind_direction = get_wind_direction_at_time(
            xq2_time, df_asiaq
        )  # Use xq2 time for wind direction
        read_and_plot(
            xq2_filepath,
            "solid",
            "blue",
    ...
    current_carra_label = read_and_plot(
        os.path.join(carra_path, carra_file),
        "solid",
        "red",
        2.5,
        1.0,
